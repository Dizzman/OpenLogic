...
-- ================================================================
CREATE OR REPLACE PROCEDURE SChaMa_EO_ItI_ToShipmentDescription_Piles_to_Ships_sp(ConfigId INT)
LANGUAGE plpgsql
AS $$
BEGIN
    ...
    SELECT
        ...
        CONCAT(RTRIM(get_part_from_to_code(ta.ShipmentDescription, 3, 6)), '|',
               RTRIM(qc.QualityCode), '|', RTRIM(qc.QualityName)),
        pq.Value
    FROM T_EO_ItI_TransferActivity ta
    JOIN T_PileQuality pq ON pq.PileId = CAST(ta.Tag2 AS INTEGER)
    JOIN T_QltCharacteristics qc ON qc._ScenarioID = ta._ScenarioID AND qc.id = pq.QualityId
    WHERE ta._ScenarioID = ConfigId
          AND ta.ObjectName = 'Piles_to_Ships'
          AND pq.Value > 0; -- exclude zero yield_SS_14.11.2017
